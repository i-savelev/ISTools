...
namespace Plugin
{
    ...
    public class TypesRename : IExternalCommand
    {
        ...
        public static string IS_IMAGE => "Plugin.Resources.TypesRename32.png";
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            TypeLayers window = new TypeLayers();
            window.dataGridView2.DataSource = dtElems;
            window.Text = "Менеджер многослойных конструкций";

            window.textBox4.Text = "Плагин отображает структуру многослойных элементов из диспетчера проекта и позволет переименовать эти элементы. Также плагин позволяет автоматически нумировать выбранные элементы. Номер присваимвается марке, например, ПГ_4.№. Для нумироции необходимо выделить строчки на жакать кнопку \"Перенумировать элементы\"";

            window.dataGridView2.CellClick += SetLayersToTable;
            window.button2.Click += (s, e) => { Rename(); };
            window.button3.Click += (s, e) => { Renumber53(); };
            window.button2.Text = "Переименовать элемент";
            window.button3.Text = "Перенумеровать элементы";
            ...
            void Renumber53()
            {
                using (Transaction tx = new Transaction(doc))
                {
                    ...
                    foreach (DataGridViewRow cell in window.dataGridView2.SelectedRows)
                    {
                        var colindex = window.dataGridView2.Columns["id"].Index;
                        var rowindex = cell.Index;
                        var namelist = window.dataGridView2["Название", rowindex].Value.ToString().Split('_');
                        string b = namelist[1];
                        string mark = $"{namelist[0]}_{b.Split('.')[0]}";
                        ...
                        string number = $"{b.Split('.')[0]}.{map[mark]}";
                        namelist[1] = number;
                        string name = string.Join("_", namelist);
                        long id = (long)Convert.ToDouble(window.dataGridView2["id", rowindex].Value);
                        Element el = doc.GetElement(new ElementId(id));
                        if (rowindex < 0) return;
                        window.dataGridView2["Название", rowindex].Value = name;
                        el.Name = name;
                        map[mark]++;
                    }
                    tx.Commit();
